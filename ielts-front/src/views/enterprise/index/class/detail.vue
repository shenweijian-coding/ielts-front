  ...
  <div>
    ...
    <div v-if="state.activeTab == 1" class="mt-4">
      <el-form :model="state.form" size="small" label-width="70" :inline="true">
        ...
        <el-form-item>
          <el-select v-model="state.form.bookId" placeholder="选择书籍" style="width: 180px" @change="getChapter" clearable
            filterable size="small">
            <el-option v-for="item in state.booksList" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item>
          <el-select v-model="state.form.chapterId" placeholder="先选择书籍，再选择章节" style="width: 180px" @change="getStudentInfo"
          ...
          <el-table-column prop="name" label="姓名" width="" align="left">
            <template #default="scope">
              <div class="flex items-center">
                ...
                <el-input v-else size="small" v-model="scope.row.name" @enter="editRowFinish(scope.row)"
                  style="width: 100px" placeholder="请输入备注" />&nbsp;
    ...
    <div v-if="state.activeTab == 2">
      <div class="flex mt-8 px-1 pb-4 flex-wrap justify-between lg:justify-initial md:justify-initial">
        <div v-for="item in state.booksList" :key="item.id"
          ...
          <div class="relative ml-1 lg:mt-2 flex h-full w-full flex-col items-start justify-start">
            <h1 class="mb-1.5 text-xl font-normal text-gray-800 group-hover:color-theme dark:text-gray-200">{{ item.name
            }}</h1>
            <p class="mb-2 font-bold text-gray-600 dark:text-gray-200">{{ item.word_total }} 词</p>
            <p class="mb-0.5 font-bold text-gray-600 dark:text-gray-200" v-if="item.chapter_total">{{ item.chapter_total
            }} 章</p>
            <div class="flex w-full items-center"><img src="@/assets/images/book.png"
                class="absolute right-3 top-3 w-16 opacity-20" /></div>
          </div>
        </div>
        <div
